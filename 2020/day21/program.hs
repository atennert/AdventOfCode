...
part2 (ingredients, map') = combine $ M.elems $ collate M.empty $ filterInAllergents $ getAllergents map'
  where mayhaveAllergent = foldr1 S.intersection
        withAllergent = M.foldr (\i s -> S.union s $ mayhaveAllergent i) S.empty map'
        ...
        collate nm om | M.null om = nm
                      | otherwise = collate (singleIng nm om) $ cleanUp (singleIng nm om) (nonSingle om)
        singleIng nm = M.union nm . M.map (head . S.toList) . M.filter ((== 1) . S.size)
        nonSingle = M.filter ((> 1) . S.size)
        cleanUp nm om = M.map (S.filter (not . (`elem` M.elems nm))) om
